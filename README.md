...
- 社員情報、所属部署、役職の管理（氏名訂正・退職登録を含む）
- 研修の登録・割り当て・進捗管理（教材ファイル添付、閲覧対象者の限定）
- 研修受講の申請・承認（上長・人事の承認フロー、承認者委任、高額・重要研修の多段階承認）
- 受講状況の確認と履歴管理（上長は自身／部下を分けて確認可能）
- 部署別の年度予算管理と消費額の可視化
- 研修効果測定（受講後アンケート・簡易テスト）と簡易ROI指標の算出
- 主要モデルへの変更操作を記録する監査ログ
- 権限に応じた業務制御（人事＝全権限、監査＝閲覧専用ロールを含む）
...
（2026-09-13: レビュー・テストの補佐にもClaude Codeの活用を追加）