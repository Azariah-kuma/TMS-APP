# VPC・サブネット・NAT Gateway・ルートテーブルを作成するモジュール。
#
# 構成:
#   - パブリックサブネット（ALB, NAT Gateway用）
#   - プライベートサブネット（ECSタスク・RDS用。インターネットからは直接到達不可）
# アベイラビリティゾーンごとに1組ずつ作成する。

locals {
  az_count = length(var.availability_zones)

  # 例: 10.0.0.0/16 → パブリック 10.0.0.0/20, 10.0.16.0/20... / プライベート 10.0.128.0/20, 10.0.144.0/20...
